...
my_data_table(mortality_data %>% 
                select(Mother_strain, Father_strain, Cross, Lineage, Block, Week, Generation, Evolution_treatment, Female_mortality, Male_mortality))
...
We fit a binomial model with `Female_mortality` as the response, `Evolution_treatment` as a fixed effect and `Week`, `Lineage` and `Cross` as random effects.
...
First with the female mortality associated extinctions included as extinction events that reflect mutation load

```{r}
extinction_model_sm_full <-
  brm(data = extinction_data_wrangled,
      ...
      seed = 1, control = list(adapt_delta = .9, max_treedepth = 10),
       file = "Fits/extinction_model_full")
```

Censor these mortality event associated extinctions and refit the model. Fit and compare with the multi-membership model.
...
ext_p1_B1 <- 
  ...
  scale_fill_manual(values = met.brewer("Hiroshige", 5)) +
  labs(x = "Generations till extinction", y = "Selection response history") +
...
(weibull_surv_plot_B1 / (ext_p1_B1 + ext_p2_B1)) / (Leaf_plot_mc_B1 + Leaf_plot_mf_B1 + Leaf_plot_fc_B1) +
  plot_annotation(tag_levels = 'a')
```
**Figure 1**. Panel **a** shows extinction trajectories under increasing stress from inbreeding. The black dotted line is the probability that a randomly selected autosomal locus remains heterozygous. **b** shows the posterior distribution of the median with associated 66 and 95% credible intervals for the number of generations that lineages with different selection response histories survived inbreeding exposure. **c** shows the distribution of the difference between each selection response history in generations survived. **d-f** 'leaf plots' that depict the difference in the proportion of lineages surviving in each generation. The innermost colour band approximates the median, while the middle and outermost bars show 66 and 95% credible intervals.   
...
my_data_table(Productivity_data_clean %>% 
                select(Mother_strain, Father_strain, Cross, Lineage, Block, Week, Generation, Treatment, Vial_setup, Count_conditions, Female_offspring, Male_offspring, Collection_window_offspring, Pre_window_female_offspring, Pre_window_male_offspring, Pre_window_offspring, Total_female_offspring, Total_male_offspring, Total_offspring))
...
`Vial setup`: specifies whether a vial was setup for a given lineage in a given generation. Vials were not setup when the lineage was extinct, or when it could no longer be included in the experiment due to escape/ handling error.
...
# Full data, no mortality removals
...
point_plot_1 <-
  ...
  # All the data
  geom_smooth(size = 1.5, colour = met.brewer("OKeeffe2")[4]) +
  geom_smooth(data = Productivity_data_clean %>% filter(Vial_setup == "Yes"), size = 1.5, colour = met.brewer("OKeeffe2")[4]) +
  # Mortality events and their consequences removed
  geom_smooth(data = Productivity_data_clean_2 %>% filter(Vial_setup == "Yes"), size = 1.5, colour = met.brewer("OKeeffe2")[6], linetype = 2) +
  geom_smooth(data = Productivity_data_clean_2, size = 1.5, colour = met.brewer("OKeeffe2")[6], linetype = 2) +
  theme_tidybayes() +
  scale_y_continuous(breaks = c(0, 25, 50, 75, 100, 125, 150), limits = c(0,150), expand = expansion(mult = c(0.01, .01))) +
...
#Figure S2**. Inbreeding depression affects lineage productivity. The plot shows offspring counted during the offspring collection window,where the light curves shows the decline in productivity with extinct lineages counted as zeros (the steeper curve), while and the decline when only extant lineages were considered (the shallower curve). The dotted curves show the same data, except with productivity counts affected by female mortality events filtered out. 
...
#What effect does evolutionary history appear to have on productivity decline?
...
treatment_plot <-
  ...
  geom_jitter(colour = met.brewer("OKeeffe2")[2], width = 0.25, 
              shape = 1.5, stroke =2, size = 1.2, alpha = 0.25) +
  geom_smooth(aes(colour = Treatment), size = 1.5) +
  scale_colour_manual(values = met.brewer("Hiroshige", 3), guide = "none") +
  theme_tidybayes() +
  scale_y_continuous(limits = c(0,150), expand = expansion(mult = c(0.01, .01))) +
  scale_x_continuous(limits = c(1,20), expand = c(0.01, 0)) +
  labs(y = "Collection window\noffspring production", x = "Generation of inbreeding") +
  theme(text = element_text(size = 14))
...
nonlinear_productivity <- 
  brm(bf(Collection_window_offspring ~ exp(a - (lambda * Generation)),
         a ~ 1 + Treatment + (1|Week) + (1|Lineage), 
...
# with female mortality events removed

nonlinear_productivity_mortality <- 
  brm(bf(Collection_window_offspring ~ exp(a - (lambda * Generation)),
         a ~ 1 + Treatment + (1|Week) + (1|Lineage), 
         lambda ~ 1 + Treatment + (1|Week) + (1|Cross), 
         nl = TRUE),
      data = Productivity_data_clean_2,
      prior = c(prior(normal(4.2, 0.25), class = b, coef = Intercept, nlpar = "a"),
                prior(normal(0, 0.25), class = b, coef = TreatmentFemale, nlpar = "a"),
                prior(normal(0, 0.25), class = b, coef = TreatmentMale, nlpar = "a"),
                prior(gamma(0.8, 4), class = b, coef = Intercept, nlpar = "lambda"),
                ...
                prior(gamma(2, 1.5), class = sd, nlpar = "a"),
                prior(gamma(0.25, 2), class = sd, nlpar = "lambda")),
      control = list(adapt_delta = 0.9, max_treedepth = 14), seed = 5,
      iter = 20000, warmup = 5000, chains = 4, cores = 4,
      file = "Fits/non_linear_productivity_model_mortality")
...
# this code finds a (initial productivity) which is technically gen 0. 

#Initial_productivity_estimates <-
#as_draws_df(nonlinear_productivity_mortality, variable = "^b_a", regex = TRUE) %>% 
 # mutate(`Female-limited` = b_a_Intercept + b_a_TreatmentFemale,
  #       `Male-limited` = b_a_Intercept + b_a_TreatmentMale,
   #      Control = b_a_Intercept) %>%
...
Initial_productivity_plot <-
  ...
  labs(x= "Productivity in generation 1", y = "Selection response history") +
  theme_bw() + 
  theme(panel.background = element_rect(fill='transparent'), #transparent panel bg
        plot.background = element_rect(fill='transparent', color=NA), #transparent plot bg
        panel.grid.minor.x = element_blank(),
...
**Figure 2**. Panel **a** shows productivity decline under increasing stress from inbreeding, split by selection response history. Bands are 50% intervals to aid visual inspection. Sand coloured points show raw productivity counts for each lineage in each generation. **b** shows the posterior distribution of median productivity in the first generation of the experiment, with associated 66 and 95% credible intervals. **c** shows the distribution of the decay parameter $\lambda$ for each selection response history. **d-f** 'leaf plots' that show the difference contrasts for productivity in each generation. The innermost colour band approximates the median, while the middle and outermost bars show 66 and 95% credible intervals.   


```{r, include=FALSE}

#### The model without extinction inclusions

#In the above model, we likely **overestimate** $\lambda$ by including extinct lineages as 0 counts in each generation. We choose to do this to negate the affect of survivor bias on our counts i.e. more productive lineages are more likely to survive for more generations in our experiment, which inflates productivity estimates in late generations and leads to **underestimation** of inbreeding depression.

#We attempt to minimise both over- and underestimation of $\lambda$ by running another model on a trimmed dataset that 1) only counts the effect of extinction once per lineage and 2) includes just the first 10 generations of each block, during which our sample size of non-zero productivity counts is still sizeable. 
  
trimmed_data <- 
  Productivity_data_clean %>% 
  filter(Vial_setup == "Yes",
         Generation < 11)

nonlinear_productivity_trimmed <- 
  brm(bf(Collection_window_offspring ~ exp(a - (lambda * Generation)),
         a ~ 1 + Treatment + (1|Week) + (1|Lineage), 
         lambda ~ 1 + Treatment + (1|Week) + (1|Cross), 
         nl = TRUE),
      ...
      prior = c(prior(normal(4, 0.5), class = b, coef = Intercept, nlpar = "a"),
                prior(normal(0, 0.25), class = b, coef = TreatmentFemale, nlpar = "a"),
                prior(normal(0, 0.25), class = b, coef = TreatmentMale, nlpar = "a"),
                prior(lognormal(-2.3, 1), class = b, coef = Intercept, nlpar = "lambda"),
                prior(normal(0, 0.1), class = b, coef = TreatmentFemale, nlpar = "lambda"),
                prior(normal(0, 0.1), class = b, coef = TreatmentMale, nlpar = "lambda"),
                prior(gamma(2, 1.5), class = sd, nlpar = "a"),
                prior(gamma(0.25, 2), class = sd, nlpar = "lambda")),
      control = list(adapt_delta = 0.999, max_treedepth = 12), seed = 1,
      iter = 10000, warmup = 6000, chains = 4, cores = 4,
      file = "Fits/non_linear_productivity_model_trimmed")


# mortality removed model

nonlinear_productivity_trimmed_mortality <- 
  brm(bf(Collection_window_offspring ~ exp(a - (lambda * Generation)),
         a ~ 1 + Treatment + (1|Week) + (1|Lineage), 
         lambda ~ 1 + Treatment + (1|Week) + (1|Cross), 
         nl = TRUE),
      ...
      control = list(adapt_delta = 0.999, max_treedepth = 12), seed = 1,
      iter = 10000, warmup = 6000, chains = 4, cores = 4,
      file = "Fits/non_linear_productivity_model_trimmed_mortality")
```
