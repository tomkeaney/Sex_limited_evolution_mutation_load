...
`Week`: the week that the generation of each block was conducted in. Note that blocks overlap between weeks 9-20.
...
$\alpha$ (the intercepts) ~ Normal($\mu$ = 0, $\sigma$ = 2)
...
$\alpha$ (the intercepts) ~ Gamma(shape = 5, rate = 3.5)
...
extinction_model_mm <-
  brm(data = extinction_data_wrangled,
      ...
      prior = c(prior(gamma(5, 3.5), class = Intercept), 
                ...
                prior(normal(0, 0.5), class = b, dpar = shape)), 
      iter = 8000, warmup = 4000, chains = 4, cores = 4,
      seed = 1, control = list(adapt_delta = 0.98, max_treedepth = 15),
...
(weibull_surv_plot_B1 / (ext_p1_B1 + ext_p2_B1)) / (Leaf_plot_mc_B1 + Leaf_plot_mf_B1 + Leaf_plot_fc_B1) +
  plot_annotation(tag_levels = 'a')
```
**Figure 1**. Panel **a** shows extinction trajectories under increasing stress from inbreeding. **b** shows the posterior distribution with mean and associated 66 and 95% credible intervals for the average number of generations that lineages with different selection response histories survived inbreeding exposure. **c** shows the distribution of the difference between each selection response history in generations survived. **d-f** 'leaf plots' that visualise these contrasts in more detail. These leaf plots depict the difference in the proportion of lineages surviving for each generation. The innermost colour shows the mean, while the middle and outmost bars show 66 and 95% credible intervals.   
...
Productivity_data_clean_2 <-
  ...
  mutate(Vial_setup = if_else(Generation > Gens_to_extinct, "No", "Yes"),
         Collection_window_offspring = case_when(
...
point_plot_1 <-
  ...
  geom_smooth(data = Productivity_data_clean_2 %>% filter(Vial_setup == "Yes"), size = 1.5, colour = met.brewer("OKeeffe2")[6]) +
  theme_tidybayes() +
...
**Figure S2**. Inbreeding depression in lineage productivity. **a** shows offspring counted during the offspring collection window, while **b** shows these offspring summed with early emerging offspring that preceded the collection window. The dark curve shows the decline in productivity with extinct lineages counted as zeros, while the lighter curve shows the decline when only extant lineages were considered. 
$~$
...
$\alpha$ (initial productivity) ~ Normal($\mu$ = 4.5, $\sigma$ = 0.5)

The effect of Block 2 on $\alpha$ also needs a prior when using this syntax

$\alpha_{Block}$ ~ Normal($\mu$ = 0, $\sigma$ = 0.5)

$\lambda$ (the rate of decay) ~ Normal($\mu$ = 0.2, $\sigma$ = 0.1) 

The effect of Block 2 on $\alpha$ also needs a prior when using this syntax

$\lambda_{Block}$ ~ Normal($\mu$ = 0, $\sigma$ = 0.1)

$\sigma$ for $\alpha$ ~ Gamma(shape = 3, rate = 1.5)

$\sigma$ for $\lambda$ ~ Gamma(shape = 0.1, rate = 2)
...
prior <-
  tibble(index = 1:n,
         ...
         lambda     = rnorm(n, mean = 0.3, sd = 0.2)) %>% 
...
prior <-
  tibble(index = 1:n,
         a   = rnorm(n, mean = 5.48, sd = 0.18),
         lambda     = rnorm(n, mean = 0.6, sd = 0.04)) %>% 
  slice_sample(n = n_draws) %>% 
  expand(nesting(index, a, lambda),
         Generation = seq(from = 0, to = 20, length.out = 1e2)) %>% 
  mutate(productivity = exp(a - lambda * Generation)) 
...
nonlinear_productivity <- 
  brm(bf(Collection_window_offspring ~ exp(a - (lambda * Generation)),
      ...
      prior = c(prior(normal(4, 0.2), class = b, coef = Intercept, nlpar = "a"),
                prior(normal(0, 0.25), class = b, coef = TreatmentFemale, nlpar = "a"),
                prior(normal(0, 0.25), class = b, coef = TreatmentMale, nlpar = "a"),
                prior(normal(0.2, 0.1), class = b, coef = Intercept, nlpar = "lambda"),
      ...
      control = list(adapt_delta = 0.8, max_treedepth = 12), seed = 1,
      iter = 8000, warmup = 4000, chains = 4, cores = 4,
      file = "Fits/non_linear_productivity_model")
